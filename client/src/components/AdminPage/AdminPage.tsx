                    ...
                    {selector === "customers" && (
                        <div className="filter">
                            ...
                            <Text type={"p-big"} textAlign={"center"}>
                                TOT:
                                {customers &&
                                    customers.filter((customer: CustomerProps) =>
                                        selectedEvent ? customer.corso === selectedEvent : customer
                                    ).length}
                                {selectedEvent &&
                                    courses &&
                                    `/${courses.filter((course: CourseProps) => course.title === selectedEvent).map((course: CourseProps, index: number) => course.details.placesAvailable)}`}
                            </Text>
                        </div>
                    )}
